...
- 任务进行中显示当前累计平均输出速度（Decode Token ÷ Decode 持续时间，不包含 Prefill/Think 等待）
- 任务结束后保留上一完整任务的平均输出速度与最高瞬时输出速度
- 本次任务输出 Token 与 vLLM 累积输出 Token
- 每张 NVIDIA GPU 的利用率、显存、温度、当前功耗和功耗上限
- 服务器 CPU、内存、网络收发带宽和磁盘读写 IO
- 独立的 Prefill、Decode 吞吐趋势（左右并排展示）
- 独立动态 Y 轴：Prefill 默认 2000 tok/s、Decode 默认 100 tok/s，超出后按当前峰值的 1.2 倍扩展
- 吞吐趋势默认保存 600 个采样点（任务活跃时约 5 分钟，空闲时约 10 分钟）
